...
Pure. Recompute `compiled` once per song load on each device (it never travels — same as the batch path); the loader computes `programHash` from the same inputs. The reducer is the only authority gate. **Program-pinned (D10):** `compiled` is an *external* argument whose `bars` array gives `vm.cursor` its meaning, so the reducer **fails closed** unless the local program, the state, and the message all name the same roadmap revision — two distinct failures with two distinct behaviors (Codex R4 LOW):
- **Local mismatch** (`programHash !== state.programHash`, the loader handed a `compiled` that doesn't match the state it's reducing) is a **programmer-invariant violation, not a runtime condition** — the reducer **throws before any admission logic** (a pre-condition `assert`). It must NOT silently proceed or coerce to a typed outcome; a wrong `compiled` would corrupt every index. This is the very first line of `reduceConductor`.
- **Message mismatch** (`msg.programHash !== state.programHash`, a cross-revision message on a correctly-loaded device) is a **runtime** condition → `{ ignored }` (§3.1.1). **Deltas, not snapshots:** every non-claim message mutates the prior state, so admission must guarantee *contiguity* — a follower may never apply delta N+2 over state N (HIGH-1). Any gap routes to `needsSnapshot`, the **same recovery door as a fresh join** (§3.3).

### 3.1 Admission (the gate) — applied first, to every message

In order. Each rule returns one `ReduceOutcome`; only the last admits.

1. **Session / song / program scope (MED-1, D10).** `msg.sessionId !== state.sessionId || msg.songRef !== state.songRef || msg.programHash !== state.programHash` → `{ ignored }` (state unchanged). The pure reducer fails closed; it does not trust transport to scope. `programHash` is what stops a same-`songRef` message from a *different roadmap revision* from being silently applied to these numeric indices (Codex R3 HIGH). (Precondition, checked *before* this rule: the local `programHash` argument must equal `state.programHash` — else the reducer **throws** (programmer error, the loader compiled the wrong structure); see §3 intro.)
...
- **D10 — Pin the exact compiled program with `programHash` on state + envelope; fail closed on any mismatch** (Codex R3 HIGH). `vm.cursor` is a numeric index into `compiled.bars` and every counter/`fired` flag keys to *that* roadmap, so `songRef` (song identity) is not enough — a structure revision under the same song silently remaps every index. `programHash = hash(ordered bar ids + roadmap markers)`, loader-computed alongside `compiled`. A session is bound to an **immutable `(songRef, programHash)`**; a cross-revision message → `{ ignored }` (§3.1.1); a structure edit ends the session (everyone rejoins) rather than mutating the program live. Snapshot adoption verifies `programHash` and reloads the SongStructure first if it differs (§3.3, a 3b/loader step). Recommend YES.
  - **Canonical serialization recipe (Codex R4 LOW — pin before build, false-mismatch risk).** The hash MUST be over a deterministic encoding, or two devices compiling the same structure could disagree and fail closed needlessly: (1) a **schema/version prefix** (e.g. `"chunk3-v1\u0000"`) so the recipe can evolve without colliding; (2) **bars** = the ordered `bars[].id` list joined by a reserved separator (the array order IS the identity — it's what `cursor` indexes); (3) **markers** sorted by a total order (`barId` asc, then `kind` asc, then marker `id` asc) so input order never matters; (4) each marker serialized with a **fixed field order** per `kind`, and inner arrays normalized — `ending.barIds` in reading order (already validated contiguous), `ending.numbers` ascending. Hash the resulting byte string (SHA-256, hex). This lives in `lib/conductor-state.ts` (or a `programHash(bars, markers)` helper) and is unit-tested for stability + order-insensitivity.
...
- **Program pin (R3 HIGH/D10):** `msg.programHash !== state.programHash` → `{ ignored }`, state unchanged (a same-`songRef` message from a different roadmap revision must NOT touch the numeric indices); **local mismatch** (`programHash` arg ≠ `state.programHash`) → **throws** before admission (R4 LOW — programmer invariant, never a silent path).
- **programHash recipe (R4 LOW):** same `(bars, markers)` → identical hash regardless of marker input order; reordering markers or permuting `ending.numbers` does NOT change the hash; reordering `bars` DOES (it's the index identity); schema-prefix bump changes the hash. Stability snapshot test pins the recipe.
