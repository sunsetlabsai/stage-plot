...
| `design-chart-review-step.md` | SHIPPED-RECORD | C1–C5. `verdict` drives review (`lib/chart-review.ts`); hand edits stamp `edited`, the sheet stamps `confirmed` (`confirmSystemSplit`); `lib/chart-resegment.ts` + `lib/chart-review-sheet.ts` + `components/ChartReviewSheet.tsx` + `ChartSystemStrip.tsx`; C5's two arms in the acceptance harness. Candidates are stored / printed-implied / measured, each drawn as a picture; there is no VLM candidate because only one split per system is ever persisted, so the stored one subsumes it. ⚠ The `uncertain` population is 0 on the corpus, so the machine-flagged path is unexercised by real charts — the owner-initiated "Check this line" door is the one that runs. |
