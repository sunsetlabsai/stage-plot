...
| `design-chart-review-step.md` | SHIPPED-RECORD | All of chunk C. `verdict` drives review (`lib/chart-review.ts`); hand edits stamp `edited`, the sheet stamps `confirmed` (`confirmSystemSplit`); `lib/chart-resegment.ts` + `lib/chart-review-sheet.ts` + `components/ChartReviewSheet.tsx` + `ChartSystemStrip.tsx`; C5's two arms in the acceptance harness. ⚠ The `uncertain` population is 0 on the corpus, so the machine-flagged path is unexercised by real charts — the owner-initiated "Check this line" door is the one that runs. |
