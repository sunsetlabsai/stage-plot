...
| `design-chart-review-step.md` | PARTIAL | **C1, C2, C4-engine + C5 shipped**: `verdict` now drives review as the exclusive per-system signal (`lib/chart-review.ts`), and hand edits stamp `verdict: 'edited'` on the parent system via `markSystemEdited` (`lib/chart-calibration.ts`); `confirmed`/`edited` are in `CHART_VERDICTS`. `lib/chart-resegment.ts` + the harness's two C5 arms are live (464/464 arm 1; N+1 refused 464/464). **C3 (the sheet UI) is unbuilt** — today's review UI is still the v1 confidence stepper. §Chunk C — build spec is what they build against. |
