...
**Status:** Proposed **v2** — addresses Codex R1 NO-GO (9 findings → 3 root causes → one architectural move; see §0). Design-only; no build.
**Date:** 2026-06-22
...
Implementation reuse: `resolveRoadmap` is already pure over `(bars, markers)`, so it runs unchanged over `SongStructure`. The build delta is a **resumable/stepwise form** that (a) yields one transition at a time and (b) accepts an override before computing the next step — the same VM, turned inside-out from run-to-completion into a stepper.
...
- **"Another round" (re-enter a repeat):** jump to the repeat-start; **hold** the target's own `completedPasses` so it fires again (don't advance it toward exit); **reset descendant repeats** nested inside the target (consistent with `backJumpTo`'s nested-reset, `:683`); **do not** touch `fired` D.S./D.C. flags or sibling counters.
...
When an MD-extended or looped node completes, the **exit** is chosen by precedence:
1. **MD-specified exit** — the directive's `exit` names the out ("…to the coda," "…then the bridge"). Wins.
2. **Default exit = the canonical-next node** after the node the band was in when the change fired — resume the default forward path *as if the node had just taken its final/fall-through exit* (final ending → fall-through for a volta repeat; past the `:|` for a plain repeat; otherwise the canonical successor). **Well-defined only because `SongStructure` gives a song-level ordering** (the #1→#3 dependency).
- **Vamp/loop sub-case:** while held, play body-only / no non-final volta; take the exit ending only on release. MD may override to a named ending.
...
## 8. Open questions (still open after v2)

Closed by v2: v1's §8.1 (continuation — §3.4), §8.2 (resync pass-state — §4, inherit), §8.5 (auto-fire — §3.5 gate), the single-writer/state-machine gaps (§4/§6). Remaining:

1. **Alignment authoring + ambiguity (§2.2):** the converter-seeded `localNode → canonicalNode` confirm flow; what happens when two charts genuinely disagree on section *count* (a part omits a tag) — block alignment, or allow a partial alignment that resolves coarse refs only? How loud is "structure differs," and is coarse-fallback (snap to nearest aligned section) ever wanted over self-navigate?
2. **`SongStructure` provenance:** is the canonical structure authored once (owner), merged from the converter's per-chart extracts, or both? Who owns edits, and how does an arrangement change re-key existing chart alignments?
3. **`barOffset` span-equivalence (§2.3):** what metadata proves a local span equivalent to a canonical span (bar-count + no intervening structure?), and the exact fallback when it can't.
4. **Listener placement + clock latency (§5.1):** dedicated node vs. MD device; phase alignment across relay latency (likely invisible at bar granularity — confirm); clock behavior when the listener drops.
5. **Failover + session discovery on a backhaul-less relay (§4/§5):** the `claim` protocol details (how a new MD is discovered + accepted post-MD-death), room discovery (QR to relay / mDNS / room code). Blocks only the transport chunk.
...
OQ-1/2/3 must close before chunks 1–2; OQ-4 before chunk 5; OQ-5 before chunk 3's discovery/failover.
...
- **One canonical VM, MD-owned;** the live layer is **B2 (default + override)** over a resumable `resolveRoadmap`; redirect counter policy + exit precedence per §3.3–3.4. **Default exit = canonical-next node; MD direction overrides** (your #3 call).
