...
**Status:** DESIGN — pre-Codex. DESIGN-ONLY, no code in this pass. Builds on chunk 1 (the
...
- **`song.bpm` + `barBeats` plumbed into the hook** (chunk 1 left `baselineTempoBpm` null);
...
| `song.bpm: number \| null` | `lib/types.ts:177` | the static-BPM tempo source; `null ⇒ manual` rung |
| `timeSig.beats` (1..32) on a roadmap-spec; calibration charts carry no meter | `lib/roadmap-spec.ts:21` | `barBeats` source order: `timeSig.beats` if a spec exists, else `4` (parent §5.5) |
...
```
on each tick (reads FRESH state via a ref — §3.2):
...
### 3.2 The stale-closure problem (the central build risk) — solved by a live-ref
...
**Resolution — a single `liveRef` updated every render**, the standard React "latest value"
pattern:

```ts
const live = { session, reckoning, clockOn, bpm, barBeats, stalled };
const liveRef = useRef(live);
liveRef.current = live;               // refs are not reactive — safe to set during render
```

The interval effect is set up **once per `[enabled, clockOn]`** and its tick reads
`liveRef.current` for *fresh* state, calling the **stable** `setSession`/`setReckoning`/
`setStalled` setters. No stale closure, no per-render interval churn. (If the project's lint
disallows writing `ref.current` during render, the fallback is a deps-free
`useEffect(() => { liveRef.current = live; })` — functionally identical; flagged §12-Q3.)
...
*and* clock arrivals through the parameterised apply (reading the toggles via the same
`liveRef`), gated by `positionTrusted || clockConfidenceOk(reckoning, rung)`. Chunk 2's deferral
...
**Clearing `stalled`.** Set only by the loop (≥ 2 owed). Cleared by any **manual position
re-anchor** — `align()`, manual `advance()`, manual `commit()` — and by the disable/identity
reset. A redirect does **not** clear it. (Mechanically: those actions reset
`motionBaselineAtMs = now`, so clearing the flag alongside lets the loop resume correctly.)
...
- **`lib/use-conductor-session.ts` (thin):** add `bpm` + `barBeats` to `UseConductorArgs`; a
  `clockOn` + `stalled` `useState`; the `prevBpm` re-baseline detector (§4); the `liveRef`
  (§3.2); the `setInterval` driver effect (`driveClockTick`); clear `stalled` in
  `align`/`advance`/`commit`; reset `clockOn`?/`stalled` + interval teardown in the same two
  identity/disable spots chunk 1 resets reckoning. Surface `clockOn`, `setClockOn`, `rung`
...
- **`app/[owner]/[show]/page.tsx` (thin):** pass `bpm` (the conducted song's stated tempo) and
  `barBeats` (`timeSig.beats` from a roadmap-spec if present, else omit → default 4); wire the
  new toggle + readout props.
...
| non-4/4 + missing-meter `barBeats` fallback | **2** | `barBeats` arg / default 4 (§1, §4) |
...
**2b — the driver (`use-conductor-session.ts` + cluster + page + their tests, vitest fake
timers):**
...
- **(a) Stale closure in the interval — the #1 risk.** Addressed by `liveRef` (§3.2); the clock
  path needs none of the auto-fire toggles (it defers), shrinking the captured surface to fresh
  `session`/`reckoning`/`bpm`/`stalled`. Flagged the lint fallback (§12-Q3).
- **(b) Double-advance from React batching.** The tick reads `liveRef` (post-commit values),
  not React state mid-batch, and emits ≤1 advance then returns; the *next* tick re-reads. Two
  ticks can't both fire the same bar because the first's `setReckoning('clock')` bumps
  `barsSinceAnchor`, which the second reads via the ref after commit. Within a single tick there
  is exactly one dispatch. (Edge: two ticks fire faster than React commits the first
  `setSession` → the second reads a stale `liveRef`. Mitigation: `CLOCK_TICK_MS` ≫ a React
  commit, and `owed` is recomputed from `motionBaselineAtMs` each tick, so a missed bump
  self-corrects next tick without a double-emit of the *same* `expected` bar. Called out §12-Q2.)
...
2. **`CLOCK_TICK_MS` value.** Recommend ~80 ms (resolves every bar to 400 bpm with margin;
   ≫ a React commit). Tune in UAT (defer-with-default, parent Decision 4 style).
3. **`liveRef` write site.** `ref.current = …` during render (idiomatic) vs a deps-free effect.
   I'll use whichever the repo's lint accepts; flagging in case there's a house preference.
